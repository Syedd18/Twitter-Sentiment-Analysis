"""
Hybrid Stock Predictor with Improved Sentiment Analysis
Combines Sentiment140-trained sentiment model with stock prediction
Target: 80%+ accuracy
"""
...
class HybridAttentionLayer(nn.Module):
    """Advanced attention mechanism for hybrid model"""
    ...
    def forward(self, lstm_output):
        ...
        context_vector = torch.mean(attn_output, dim=0)
        return context_vector, attn_weights

class HybridLSTM(nn.Module):
    """Hybrid LSTM with improved sentiment integration"""
    def __init__(self, input_size, hidden_sizes=[256, 128, 64], num_layers=3, 
        ...
        if use_attention:
            self.attention = HybridAttentionLayer(current_size)
...
def create_hybrid_features(df: pd.DataFrame, sentiment_model: Any = None) -> pd.DataFrame:
    """Create hybrid features with improved sentiment"""
    df = df.copy()
    ...
    if sentiment_model and 'avg_sentiment' in df.columns:
        # Use improved sentiment model if available
        df['improved_sentiment'] = df['avg_sentiment']  # Placeholder for now
        df['sentiment_confidence'] = df['pos_rate'] + df['neg_rate']
        ...
        # Fallback to existing sentiment features
        df['sentiment_momentum'] = df['avg_sentiment'].rolling(5).mean()
        df['sentiment_volatility'] = df['avg_sentiment'].rolling(10).std()
        df['sentiment_trend'] = df['avg_sentiment'].rolling(20).mean()
...
def create_hybrid_target(df: pd.DataFrame, threshold: float = 0.015) -> pd.Series:
    """Create hybrid target with optimized threshold"""
...
def select_hybrid_features(X: np.ndarray, y: np.ndarray, feature_names: List[str], k: int = 35) -> Tuple[np.ndarray, List[str]]:
    """Select hybrid features using mutual information"""
    selector = SelectKBest(score_func=mutual_info_classif, k=k)
    X_selected = selector.fit_transform(X, y)
    selected_features = [feature_names[i] for i in selector.get_support(indices=True)]
    
    logger.info(f"Selected {len(selected_features)} hybrid features:")
...
def train_hybrid_lstm(
    ...
    """Train hybrid LSTM model with improved sentiment"""
    
    if device == 'cuda' and not torch.cuda.is_available():
        device = 'cpu'
        logger.warning("CUDA not available, using CPU")
    
    logger.info(f"Training hybrid model for {symbol} on device: {device}")
    
    # Create hybrid target
    target = create_hybrid_target(sdf)
    ...
    X_train_selected, selected_features = select_hybrid_features(
        train_scaled, train_target.values, feature_cols, k=35
    )
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_accuracy:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), f'models/hybrid_best_lstm_{symbol}.pth')
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(f'models/hybrid_best_lstm_{symbol}.pth'))
    model.eval()
    ...
    logger.info(f"Hybrid LSTM Accuracy: {final_accuracy:.4f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train hybrid stock predictor")
    ...
    df = load_features()
    df = create_hybrid_features(df, sentiment_model)
    ...
    logger.info(f"Training hybrid models on symbols: {symbols}")
    ...
    # Save results
    results_path = os.path.join(DATA_CONFIG['processed_data_dir'], 'hybrid_lstm_results.json')
    ...
    if successful_results:
        ...
        logger.info(f"\n{'='*50}")
        logger.info("HYBRID TRAINING SUMMARY")
        logger.info(f"{'='*50}")
